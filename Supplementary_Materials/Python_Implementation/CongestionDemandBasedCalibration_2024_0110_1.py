...
def CDBCalibration(_NUMBER_OF_SECOND_PER_MIN, assign_period_start_time_in_hour, assign_period_end_time_in_hour
    ...
    """
    g_link_list = list()
    with open("./input/link.csv", "r") as fp:
        reader = csv.DictReader(fp)
        for line in reader:
            link = Link(link_id=line["link_id"], V=line["assignment_period_volume"], QDF=line["QDF"], t2=line["t2"],
                        L=line["length"], C=line["ultimate_capacity"], vc=line["critical_speed"],
                        vf=line["free_flow_speed"], mm=line["flatness_of_curve"], a=line["cd_alpha"], b=line["cd_beta"])
            g_link_list.append(link)

    performance_fp = open("./output/link_performance.csv", "w", newline="")
    ...
    td_queue_fp = open("./output/td_queue.csv", "w", newline="")
    td_speed_fp = open("./output/td_speed.csv", "w", newline="")
    ...
    print("./output/link_performance.csv")
    print("./output/td_queue.csv")
    print("./output/td_speed.csv")
...
if __name__ == "__main__":
    ...
    number_of_interval = int((assign_period_end_time_in_hour - assign_period_start_time_in_hour) * 60 / time_interval_in_min) + 1
